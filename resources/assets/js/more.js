...
const blUrlDomainAndPathOnly = (window.location.origin).concat(window.location.pathname);
const blQueryString = window.location.search;
const blUrlParams = new URLSearchParams(blQueryString);
const blUrlGclid = blUrlParams.get('gclid');
const blUrlUtmSource = blUrlParams.get('utm_source');
const blUrlUtmMedium = blUrlParams.get('utm_medium');
const blUrlUtmCampaign = blUrlParams.get('utm_campaign');
const blUrlUtmContent = blUrlParams.get('utm_content');
const blUrlUtmTerm = blUrlParams.get('utm_term');
// "blCookieDuration" is defined in seconds, 1 day = 86400, 1 week = 604800, 2 weeks = 1209600, 28 days = 2419200. 2 weeks should be long enough to measure something but short enough that the data can't get get stale and buggy. The default is 0, which sets a session cookie instead of duration based cookie. The default can be overridden when using the variable in a function.
var blCookieDuration = 0;
// Default cookie to codefresh.io domain
var blCookieDomain = 'codefresh.io';
// Default cookie to path /
var blCookiePath = '/'

// Functions for getting DOM elements by Inner Text value, via https://stackoverflow.com/a/47815081
...
const toggleMobileMenu = ( trigger, menu ) => {
...
const countryCodeToName = ( country, all ) => {
  if ( country !== '' ) {
    for ( var key in all ) {
      if ( key == country ) {
        return all[ key ];
...
function animateValue ( id, start, end, duration ) {
  let range = end - start;
  let current = start;
  let increment = end > start ? start : -1;
...
(function($){
  ...
  document.addEventListener( 'DOMContentLoaded', () => {
    const GFBtns = document.querySelectorAll( selectors.GFBtn );
    ...
    const newsletterPopup = $( '.modal' );
    const $_WINDOW = $( window );
    const calcX = () => winX = $_WINDOW.width();
    const calcY = () => posY = $_WINDOW.scrollTop();
    ...
    if ( mobileMenuTrigger ) {
      mobileMenuTrigger.addEventListener( 'click', toggleMobileMenu.bind( this, mobileMenuTrigger, mobileMenu ) );
    }
    ...
    */
     ...
     */
    if ( GFBtns.length ) {
      GFBtns.forEach( btn => btn.addEventListener( 'click', GFLoading ) );
    }

    /**
     * Blog posts.
     */
    const newsletterClose = $( '.modal-close' );
...
jQuery.scrollToSection = function (target, offset, speed, container) {
    if (isNaN(target)) {
